...
export function CameraSelect(props: {
  value: string;
  permissionGranted: boolean;
}) {
  return (
    <div className="flex flex-col gap-[0.25rem] items-stretch text-[--text-primary]">
      <label className="text-[--text-tertiary] text-[0.875rem]">Camera</label>
      ...
      <div className="flex">
        <div className="w-full">
          <div className="flex flex-row items-center h-[2rem] px-[0.375rem] gap-[0.375rem] border rounded-lg border-gray-200 w-full disabled:text-gray-400 transition-colors KSelect">
            <CameraIcon className="text-gray-400 size-[1.25rem]" />
            <span className="flex-1 text-left truncate">No Camera</span>
            <button
              type="button"
              className={cn(
